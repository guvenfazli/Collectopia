...
export default function HeaderNavigator({ socket }: ComponentProps) {
  ...
  const [isCreateAuction, setIsCreateAuction] = useState<boolean>(false)
  ...
  return (
    <nav className="flex flex-row justify-around items-center gap-5 text-lg">
      ...
      <Dialog open={isCreateAuction} onOpenChange={() => setIsCreateAuction(prev => !prev)}>
        ...
        <DialogContent className="bg-orange-50 text-lg text-orange-800 flex flex-col border border-orange-800">
          ...
          <ItemsForAuctionCreation socket={socket} setIsCreateAuction={setIsCreateAuction} />
        </DialogContent>
      ...
      <TooltipProvider delayDuration={300}>
        <Tooltip>
          <TooltipTrigger asChild>
            <Link className="shadow-sm shadow-orange-800 hover:shadow-md hover:shadow-orange-900 transition-all duration-200 border border-red-900 p-2 py-2 bg-white rounded-full hover:bg-orange-300" href={'/offers'}><RiSwapFill /></Link>
          </TooltipTrigger>
          <TooltipContent>
            <p>Offers</p>
          </TooltipContent>
